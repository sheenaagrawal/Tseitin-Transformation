...
class TseitinTransformer
{
 //private:
   public:
    ...
    //std::map<int,std::string> varIdTable; // stores the ID associated with the variable
    ...
    int findValue(std::map<int,std::string> &Map, std::string K);
    bool in_array(std::string value);
